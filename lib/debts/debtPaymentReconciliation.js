...
export async function unlinkDebtPaymentReconciliation({ db, householdId, debtId, primaryPaymentId, duplicatePaymentId, userId = null }) {
  requireDbContract(db);
  ...
  if (!primaryPaymentId) throw new ReconciliationError(400, 'primaryPaymentId is required');
  if (!duplicatePaymentId) throw new ReconciliationError(400, 'duplicatePaymentId is required');
  if (primaryPaymentId === duplicatePaymentId) throw new ReconciliationError(400, 'cannot unlink a payment from itself');
  ...
  return db.transaction(async (tx) => {
    ...
    if (typeof tx.deleteDebtPaymentReconciliation !== 'function') {
      throw new ReconciliationError(500, 'reconciliation deletion is not supported by the current database adapter');
    }

    const deleted = await tx.deleteDebtPaymentReconciliation({ householdId, primaryPaymentId, duplicatePaymentId });
    if (!deleted) {
      throw new ReconciliationError(404, 'reconciliation not found for this pair');
    }
    ...
    await logAuditEvent({
      ...
      workspaceId: householdId,
      userId: userId ?? 'system',
      event: 'debt.payment_reconciliation.unlinked',
      entityId: deleted.id,
      metadata: { entityType: 'debt_payment_reconciliation', debtId },
    });
