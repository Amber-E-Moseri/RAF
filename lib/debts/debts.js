...
const adjustmentTypeSchema = z.enum(['correction', 'interest', 'fee', 'late_fee', 'reconciliation']);
...
export async function updateDebt({ db, householdId, debtId, input }) {
  ...
  return db.transaction(async (tx) => {
    ...
    const nextFinancialAccountId = Object.prototype.hasOwnProperty.call(parsedInput, 'financialAccountId')
      ...
      : existing.financialAccountId ?? null;
...
export async function unlinkDebtFromFinancialAccount({ db, householdId, debtId }) {
  return updateDebt({
    db,
    householdId,
    debtId,
