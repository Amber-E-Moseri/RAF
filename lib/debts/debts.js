...
} from './debtActivity.js';
import { findDebtPaymentMatches } from './debtPaymentMatching.js';
...
export async function listDebtActivity({ db, householdId, debtId, month = null, view = 'economic' }) {
  if (!householdId) throw new DebtHttpError(400, 'householdId is required');
  if (!debtId) throw new DebtHttpError(400, 'debtId is required');
  requireDbContract(db);

  return db.transaction(async (tx) => {
    const debt = await tx.getDebtById({ householdId, debtId });
    if (!debt) throw new DebtHttpError(404, 'debt not found');

    const [payments, adjustments, reconciliations] = await Promise.all([
      tx.listDebtPayments({ householdId, debtId }),
      typeof tx.listDebtAdjustments === 'function'
        ? tx.listDebtAdjustments({ householdId, debtId })
        : Promise.resolve([]),
      typeof tx.listDebtPaymentReconciliations === 'function'
        ? tx.listDebtPaymentReconciliations({ householdId, debtId })
        : Promise.resolve([]),
    ]);

    const rawActivities = deriveDebtActivity({ payments, adjustments });

    if (view === 'raw') {
      if (month) {
        const summary = deriveDebtMonthlyActivitySummary(rawActivities, month);
        return { view: 'raw', month, ...summary };
      }
      return { view: 'raw', activities: rawActivities };
    }

    const economic = deriveEconomicDebtActivity({ activities: rawActivities, reconciliations });

    if (month) {
      const summary = deriveDebtMonthlyActivitySummary(economic, month);
      return { view: 'economic', month, ...summary };
    }

    const matches = findDebtPaymentMatches({ activities: rawActivities, reconciliations });
    return { view: 'economic', activities: economic, reconciliations, matches };
  });
}
