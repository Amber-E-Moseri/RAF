...
const BUFFER_DISPOSITION_TYPES = ['apply_to_goal', 'apply_to_debt', 'return_to_plan'];

function validateBufferDispositionChoice(disposition) {
  const { type, targetId = null } = disposition ?? {};
  if (!BUFFER_DISPOSITION_TYPES.includes(type)) {
    throw new MonthlyReviewHttpError(400, `disposition.type must be one of: ${BUFFER_DISPOSITION_TYPES.join(', ')}`);
  }
  if (type === 'apply_to_goal' && !targetId) {
    throw new MonthlyReviewHttpError(400, 'targetId (goalId) is required for apply_to_goal disposition.');
  }
  if (type === 'apply_to_debt' && !targetId) {
    throw new MonthlyReviewHttpError(400, 'targetId (debtId) is required for apply_to_debt disposition.');
  }
  if (type === 'return_to_plan' && targetId) {
    throw new MonthlyReviewHttpError(400, 'targetId is not allowed for return_to_plan disposition.');
  }
  return { type, targetId };
}

function deriveBufferState({ allocationCategories, incomeAllocations, transactions }) {
  ...
  return {
    ...
    allocatedCents,
    spentCents,
    remainingCents: Math.max(allocatedCents - spentCents, 0),
    overrun: allocatedCents - spentCents < 0,
    overrunCents: Math.max(spentCents - allocatedCents, 0),
  };
}

async function loadBufferState({ tx, householdId, period, allocationCategories = null }) {
  const [categories, incomeAllocations, transactions] = await Promise.all([
    allocationCategories ?? tx.listAllocationCategories({ householdId, asOf: period }),
...
export async function getCloseReadiness({ db, householdId, period }) {
  ...
  return db.transaction(async (tx) => {
    const [closes, review, transactions, allocationCategories, goals, debts, debtPayments, incomeAllocations] = await Promise.all([
      typeof tx.listMonthCloses === 'function'
        ? tx.listMonthCloses({ householdId, period: normalizedPeriod })
        : [],
      tx.getMonthlyReviewByMonth({ householdId, reviewMonth: normalizedPeriod }),
      ...
      tx.listIncomeAllocations({ householdId, from: normalizedPeriod, to: normalizedPeriod }),
    ]);
    ...
    const bufferState = deriveBufferState({ allocationCategories, incomeAllocations, transactions: txRows });
    const bufferCategory = bufferState.category;
    ...
    return {
      ...
      summary: {
        ...
        bufferRemaining: bufferCategory ? formatCents(bufferState.remainingCents) : null,
...
async function buildCloseSnapshot({ tx, householdId, period }) {
  ...
  if (bufferCategory) {
    bufferSnapshot = {
      ...
      allocated: formatCents(bufferState.allocatedCents),
      spent: formatCents(bufferState.spentCents),
      remaining: formatCents(bufferState.remainingCents),
      overrun: bufferState.overrun,
      overrunAmount: formatCents(bufferState.overrunCents),
    };
  }
...
async function applyBufferDispositionInTransaction({
  tx,
  householdId,
  period,
  userId,
  ...
  const { type, targetId } = validateBufferDispositionChoice(disposition);
  if (remainingCents <= 0) {
    throw new MonthlyReviewHttpError(422, 'No buffer remaining to dispose.');
  }

  const dispositionAmount = formatCents(remainingCents);
  ...
  if (type === 'apply_to_goal') {
    const goal = typeof tx.getGoalById === 'function'
      ...
      : null;
    if (!goal || goal.householdId !== householdId || goal.active === false) {
      throw new MonthlyReviewHttpError(422, `Goal ${targetId} not found or not eligible in this workspace.`);
    }
    ...
    const debt = typeof tx.getDebtById === 'function'
      ...
      : null;
    if (!debt || debt.householdId !== householdId || debt.isActive === false) {
      throw new MonthlyReviewHttpError(422, `Debt ${targetId} not found or not eligible in this workspace.`);
    }
    ...
    appliedAction = { type: 'return_to_plan', note: 'Unused buffer remains in this month surplus.' };
  }
  ...
  await logAuditEvent({
    ...
    entityId: null,
    metadata: { period, dispositionType: type, targetId: targetId ?? null },
  });
...
export async function closeMonth({ db, householdId, period, userId, bufferDispositionInput = null }) {
  ...
  return db.transaction(async (tx) => {
    ...
    if (bufferDispositionInput) {
      const allocationCategories = await tx.listAllocationCategories({ householdId, asOf: normalizedPeriod });
      const bufferState = await loadBufferState({ tx, householdId, period: normalizedPeriod, allocationCategories });
      if (!bufferState.category) {
        throw new MonthlyReviewHttpError(422, 'No active buffer allocation found for this workspace.');
      }
      const applied = await applyBufferDispositionInTransaction({
        tx,
        householdId,
        period: normalizedPeriod,
        userId,
        disposition: bufferDispositionInput,
        remainingCents: bufferState.remainingCents,
        allocationCategories,
      });
      bufferDisposition = {
        type: applied.type,
...
export async function applyBufferDisposition({ db, householdId, period, userId, disposition }) {
  ...
  const { amount } = disposition ?? {};
  const { type, targetId = null } = validateBufferDispositionChoice(disposition);
  ...
  return db.transaction(async (tx) => {
    ...
    const remainingCents = bufferState.remainingCents;
    if (remainingCents <= 0) {
      return {
        applied: false,
