...
export async function getCloseReadiness({ db, householdId, period }) {
  ...
  return db.transaction(async (tx) => {
    ...
    // Use the same field access as applyBufferDisposition and buildCloseSnapshot:
    // allocatedAmount ?? amount for income allocations, Math.abs for debit transactions.
    const bufferRemainingCents = bufferCategory
      ? Math.max(
          incomeAllocations
    ...
    return {
      ...
      summary: {
        ...
        bufferRemaining: bufferCategory ? formatCents(bufferRemainingCents) : null,
...
export async function closeMonth({ db, householdId, period, userId, bufferDispositionInput = null }) {
  ...
  return db.transaction(async (tx) => {
    ...
    // ── Atomic buffer disposition ────────────────────────────────────────────
    // Financial effects (goal contribution / debt payment) are executed here,
    // inside this transaction, BEFORE buildCloseSnapshot. That way:
    //   (a) disposition and close are one atomic unit — either both persist
    //       or neither does, with no orphaned goal/debt transactions.
    //   (b) the close snapshot captures the disposition transaction because
    //       buildCloseSnapshot reads the same transaction state.
    //
    // The backend derives the authoritative amount; the client sends only
    // { type, targetId } — any client-supplied amount is ignored.
    let bufferDisposition = null;
    if (bufferDispositionInput) {
      const { type: dispType, targetId: dispTargetId = null } = bufferDispositionInput;
      const validTypes = ['apply_to_goal', 'apply_to_debt', 'return_to_plan'];
      if (!validTypes.includes(dispType)) {
        throw new MonthlyReviewHttpError(400, `bufferDisposition.type must be one of: ${validTypes.join(', ')}`);
      }

      // Derive authoritative remaining using the same formula as applyBufferDisposition
      const [preCats, preAllocs, preRawTxs] = await Promise.all([
        tx.listAllocationCategories({ householdId, asOf: normalizedPeriod }),
        tx.listIncomeAllocations({ householdId, from: normalizedPeriod, to: normalizedPeriod }),
        tx.listTransactions({ householdId, from: normalizedPeriod, to: normalizedPeriod }),
      ]);
      const preTxs = unwrapRows(preRawTxs);
      const bufCat = preCats.find((c) => c.isBuffer === true && c.isActive !== false) ?? null;

      if (bufCat) {
        const allocCents = preAllocs
          .filter((a) => (a.allocationCategoryId ?? a.categoryId) === bufCat.id)
          .reduce((s, a) => s + parseMoneyToCents(a.allocatedAmount ?? a.amount ?? '0.00'), 0);
        const spentCents = preTxs
          .filter((t) => t.categoryId === bufCat.id && t.direction === 'debit')
          .reduce((s, t) => s + Math.abs(parseMoneyToCents(t.amount ?? '0.00')), 0);
        const remainingCents = Math.max(allocCents - spentCents, 0);

        if (remainingCents > 0) {
          const dispositionAmount = formatCents(remainingCents);

          if (dispType === 'apply_to_goal') {
            if (!dispTargetId) throw new MonthlyReviewHttpError(400, 'targetId (goalId) is required for apply_to_goal disposition.');
            const goal = typeof tx.getGoalById === 'function'
              ? await tx.getGoalById({ householdId, goalId: dispTargetId })
              : null;
            if (!goal || goal.householdId !== householdId) {
              throw new MonthlyReviewHttpError(422, `Goal ${dispTargetId} not found in this workspace.`);
            }
            await tx.insertTransaction({
              householdId,
              transactionDate: normalizedPeriod,
              description: `Buffer disposition: ${goal.name ?? dispTargetId}`,
              amount: dispositionAmount,
              direction: 'debit',
              categoryId: goal.bucketId ?? null,
              linkedGoalId: dispTargetId,
              source: 'buffer_disposition',
            });
          } else if (dispType === 'apply_to_debt') {
            if (!dispTargetId) throw new MonthlyReviewHttpError(400, 'targetId (debtId) is required for apply_to_debt disposition.');
            const debt = typeof tx.getDebtById === 'function'
              ? await tx.getDebtById({ householdId, debtId: dispTargetId })
              : null;
            if (!debt || debt.householdId !== householdId) {
              throw new MonthlyReviewHttpError(422, `Debt ${dispTargetId} not found in this workspace.`);
            }
            const debtCat = preCats.find((c) => c.slug === 'debt_payoff') ?? null;
            const created = await tx.insertTransaction({
              householdId,
              transactionDate: normalizedPeriod,
              description: `Buffer disposition: ${debt.name ?? dispTargetId}`,
              amount: dispositionAmount,
              direction: 'debit',
              categoryId: debtCat?.id ?? null,
              linkedDebtId: dispTargetId,
              source: 'buffer_disposition',
            });
          ...
          // return_to_plan: no financial mutation — surplus already includes unused buffer

          bufferDisposition = {
            type: dispType,
            amount: dispositionAmount,
            targetId: dispTargetId,
          };
        }
      }
    }

    // Build snapshot AFTER disposition so the snapshot captures the disposition transaction
