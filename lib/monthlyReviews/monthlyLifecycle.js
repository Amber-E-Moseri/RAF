...
export async function applyBufferDisposition({ db, householdId, period, userId, disposition }) {
  ...
  const validTypes = ['roll_to_next_buffer', 'apply_to_goal', 'apply_to_debt', 'return_to_plan'];
  ...
  return db.transaction(async (tx) => {
    ...
    const nextPeriod = nextMonthPeriod(normalizedPeriod);
    ...
    if (type === 'apply_to_goal') {
      ...
      appliedAction = { type: 'transaction', id: created.id };
    } else if (type === 'roll_to_next_buffer') {
      const idempotencyKey = `buffer_carry_forward:${householdId}:${normalizedPeriod}`;
      const existingEntry = await tx.findIncomeByIdempotencyKey({ householdId, idempotencyKey });
      if (!existingEntry) {
        const carryEntry = await tx.insertIncomeEntry({
          householdId,
          receivedDate: nextPeriod,
          amount: dispositionAmount,
          source: 'carry_forward',
          description: `Buffer carry-forward from ${normalizedPeriod.slice(0, 7)}`,
          idempotencyKey,
        });
        const incomeAllocRow = await tx.insertIncomeAllocations([{
          householdId,
          incomeEntryId: carryEntry.id,
          allocationCategoryId: bufferCategory.id,
          allocatedAmount: dispositionAmount,
        }]);
        appliedAction = { type: 'carry_forward_income', incomeEntryId: carryEntry.id, period: nextPeriod };
      } else {
        appliedAction = { type: 'carry_forward_income_existing', incomeEntryId: existingEntry.id, period: nextPeriod };
      }
    ...
    return {
      ...
      nextPeriod: type === 'roll_to_next_buffer' ? nextPeriod : null,
      appliedAction,
    };
  });
}
