...
export function extractFinancialDecisions(activityRows) {
  return activityRows
    ...
    .map((row) => {
      // Metadata is jsonb in production (always an object or null).
      // Guard against unexpected non-object values (e.g. a JSON string) so
      // malformed rows never crash the report.
