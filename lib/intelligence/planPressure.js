...
function normalizeMonth(row) {
  return row.reviewMonth ?? row.review_month;
}

function categorySlugByIdForMonth(categories, month) {
  const byId = new Map();
  for (const cat of categories) {
    const from = cat.effectiveFrom ?? cat.effective_from ?? null;
    const superseded = cat.supersededAt ?? cat.superseded_at ?? null;
    if (!from || from > month) continue;
    if (superseded && superseded <= month) continue;
    const id = cat.id;
    if (id && cat.slug) byId.set(id, cat.slug);
  }
  return byId;
}
 ...
 * @param {Map}     [opts.splitsByTxId]   Map<transactionId, SplitRow[]> for split-aware attribution
 */
export function buildClosedMonthsForPressure({
  ...
  return monthlyReviews.map((review) => {
    const month = normalizeMonth(review);
    ...
    const effectiveSlugById = categorySlugByIdForMonth(effectiveCategories, month);
    for (const allocation of allAllocations) {
      const entryMonth = (allocation.receivedDate ?? allocation.received_date ?? '').slice(0, 7);
      if (entryMonth !== month.slice(0, 7)) continue;
      const catId = allocation.allocationCategoryId ?? allocation.allocation_category_id;
      const slug = catId ? effectiveSlugById.get(catId) : null;
      if (!slug) continue;
      categoryPlanned.set(
        slug,
        (categoryPlanned.get(slug) ?? 0) + parseMoneyToCents(allocation.allocatedAmount ?? allocation.allocated_amount ?? 0),
      );
    }

    if (categoryPlanned.size === 0 && totalIncomeCents > 0) {
    ...
    // Split-aware: if a transaction has splits, attribute each split's amount
    // to its split.categoryId (resolved to slug). Avoids double-counting the
    // parent transaction amount when splits cover the full amount.
    ...
    for (const tx of allTransactions) {
      ...
      const txId = tx.id;
      if (splitsByTxId.has(txId)) {
        // Split transaction: attribute each split to its own category.
        for (const split of splitsByTxId.get(txId)) {
          const catId = split.categoryId ?? split.category_id;
          const slug = catId ? (effectiveSlugById.get(catId) ?? null) : null;
          if (!slug) continue;
          categoryActuals.set(slug, (categoryActuals.get(slug) ?? 0) + parseMoneyToCents(split.amount));
        }
      } else {
        // Unsplit: resolve categoryId → slug.
        const catId = tx.categoryId ?? tx.category_id;
        const slug = catId ? (effectiveSlugById.get(catId) ?? tx.categorySlug ?? tx.category_slug ?? null) : null;
        if (!slug) continue;
        categoryActuals.set(slug, (categoryActuals.get(slug) ?? 0) + parseMoneyToCents(tx.amount));
      }
    }
