...
import { checkPdfImportQuota, recordPdfImportUsage } from './quotaHelper.js';
import {
  ImportHttpError,
  normalizeIsoDate,
  normalizeMoney,
...
export async function importBankStatement({ db, householdId, input, pdfTextExtractor }) {
  ...
  if (rawExtracted.length === 0) {
    ...
    const quotaStatus = await checkPdfImportQuota(db, householdId);
    if (!quotaStatus.canImport) {
      throw new ImportHttpError(
        429,
        `PDF import quota exceeded. Remaining: 0/${quotaStatus.tier === 'free' ? 3 : '?'}. Upgrade to paid plan for unlimited imports.`,
        { remaining: 0, tier: quotaStatus.tier },
      );
    }
    ...
    const aiResult = await parseWithAI(pdfText, { apiKey });
