...
function buildDirectTransaction(client) {
  return {
    ...
    async updateWorkspace({ workspaceId, patch }) {
      ...
      const updated = { ...workspace, ...patch, updatedAt: isoNow() };
      await client.query(
        `update ${POSTGRES_SCHEMA}.workspaces set name = $2, timezone = $3, country = $4, default_currency = $5, updated_at = $6, raw_json = $7 where id = $1`,
    ...
    async createPasswordResetToken({ userId, tokenHash, expiresAt }) {
      const result = await client.query(
        `INSERT INTO raf.password_reset_tokens (user_id, token_hash, expires_at)
         VALUES ($1, $2, $3)
    ...
    async findValidPasswordResetToken({ tokenHash }) {
      const result = await client.query(
        `SELECT id, user_id, token_hash, expires_at, consumed_at, created_at
         FROM raf.password_reset_tokens
         WHERE token_hash = $1
    ...
    async consumePasswordResetTokenAndUpdatePassword({ tokenHash, userId, newPasswordHash }) {
      ...
      const check = await client.query(
        `SELECT id FROM raf.password_reset_tokens
         WHERE token_hash = $1 AND user_id = $2 AND consumed_at IS NULL AND expires_at > now()
         FOR UPDATE`,
        [tokenHash, userId],
      );
      if (!check.rows.length) return false;
      // Invalidate ALL outstanding tokens for this user (including the one being consumed).
      await client.query(
        `UPDATE raf.password_reset_tokens
         SET consumed_at = now()
      ...
      const upd = await client.query(
        `UPDATE raf.app_users SET password_hash = $1, updated_at = now()
         WHERE id = $2`,
    ...
    // Atomically reserve one free-tier AI parse slot.
    // Uses INSERT ON CONFLICT DO UPDATE WHERE count < FREE_LIMIT so the limit is
    // enforced inside a single statement; PostgreSQL row-level locking serializes
    // concurrent callers at the last slot without any advisory lock or loadState.
    // Returns { allowed: true, ... } when a slot was reserved, { allowed: false, ... }
    // when count >= FREE_LIMIT.  Paid-tier admission does not go through this path.
        ...
        `insert into ${POSTGRES_SCHEMA}.pdf_import_quotas
         ...
         on conflict (workspace_id, year_month) do update
           set count    = ${POSTGRES_SCHEMA}.pdf_import_quotas.count + 1,
               raw_json = ${POSTGRES_SCHEMA}.pdf_import_quotas.raw_json
                          || jsonb_build_object('count', ${POSTGRES_SCHEMA}.pdf_import_quotas.count + 1)
           where ${POSTGRES_SCHEMA}.pdf_import_quotas.count < $5
         returning count`,
  ...
  return {
    ...
    async transaction(callback, securityContext = {}) {
      ...
      try {
        ...
        const result = await callback(directTx);
