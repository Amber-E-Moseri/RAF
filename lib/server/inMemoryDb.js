...
function invitationParityError(code, detail) {
  const err = new Error(code + (detail != null ? ': ' + detail : ''));
  err.isInvitationError = true;
  err.code = code;
...
export function createInMemoryDb() {
  ...
  const tx = {
    ...
    async deleteWorkspaceById({ workspaceId }) {
      ...
      state.monthlyCloses = state.monthlyCloses.filter((row) => row.householdId !== workspaceId && row.workspaceId !== workspaceId);
      state.upcomingExpenses = state.upcomingExpenses.filter((row) => row.householdId !== workspaceId && row.workspaceId !== workspaceId);
      return true;
    },
    ...
    async getWorkspaceInvitationByToken({ token }) {
      const row = state.workspaceInvitations.find((r) => r.token === token);
      if (!row) return null;
      if (row.status === 'pending' && row.expiresAt && row.expiresAt < isoNow()) {
        row.status = 'expired';
    ...
    async acceptWorkspaceInvitation({ tokenHash, userId, userEmail }) {
      const inv = state.workspaceInvitations.find((r) => r.token === tokenHash);
      if (!inv) throw invitationParityError('INV_NOT_FOUND');
      if (inv.status === 'pending' && inv.expiresAt && inv.expiresAt < isoNow()) {
        inv.status = 'expired';
        inv.updatedAt = isoNow();
        throw invitationParityError('INV_EXPIRED');
      }
      if (inv.status !== 'pending') throw invitationParityError('INV_NOT_PENDING', inv.status);
      if (inv.email !== String(userEmail ?? '').trim().toLowerCase()) {
        throw invitationParityError('INV_EMAIL_MISMATCH');
      }
      const alreadyMember = state.workspaceMembers.find(
        (r) => r.workspaceId === inv.workspaceId && r.userId === userId && r.status === 'active',
      );
      if (alreadyMember) throw invitationParityError('INV_ALREADY_MEMBER');
    ...
    async createPasswordResetToken({ userId, tokenHash, expiresAt }) {
      const row = { id: uuid(), userId, tokenHash, expiresAt, consumedAt: null, createdAt: isoNow() };
      state.passwordResetTokens.push(row);
      return clone(row);
    },
    async findValidPasswordResetToken({ tokenHash }) {
      const now = isoNow();
      const row = state.passwordResetTokens.find(
        (r) => r.tokenHash === tokenHash && r.consumedAt === null && r.expiresAt > now,
      );
      return row ? clone(row) : null;
    },
    async consumePasswordResetTokenAndUpdatePassword({ tokenHash, userId, newPasswordHash }) {
      const now = isoNow();
      const token = state.passwordResetTokens.find(
        (r) => r.tokenHash === tokenHash && r.consumedAt === null && r.expiresAt > now,
      );
      if (!token) return false;
      // Consume this token and invalidate all others for this user atomically.
      for (const r of state.passwordResetTokens) {
        if (r.userId === userId && r.consumedAt === null) {
          r.consumedAt = now;
        }
      }
    ...
    async insertIncomeEntry(payload) {
      if (payload.idempotencyKey) {
        const existing = state.incomeEntries.find(
          (r) => r.householdId === payload.householdId && r.idempotencyKey === payload.idempotencyKey,
        );
    ...
    async listTransactions({ householdId: targetHouseholdId, from, to, categoryId = null, categoryIds = null, direction = null, reviewed = null, cursor = null, limit = 50 }) {
      const items = state.transactions
        ...
        .filter((row) => {
          if (reviewed === false) return !row.reviewedAt;
          if (reviewed === true) return Boolean(row.reviewedAt);
          return true;
        })
    ...
    async markTransactionReviewed({ householdId: targetHouseholdId, transactionId, reviewedAt, reviewedBy }) {
      const row = state.transactions.find((entry) => entry.householdId === targetHouseholdId && entry.id === transactionId);
      if (!row) return null;
      Object.assign(row, { reviewedAt, reviewedBy: reviewedBy ?? null, updatedAt: isoNow() });
      return clone(row);
    },
    async markTransactionUnreviewed({ householdId: targetHouseholdId, transactionId }) {
      const row = state.transactions.find((entry) => entry.householdId === targetHouseholdId && entry.id === transactionId);
      if (!row) return null;
      Object.assign(row, { reviewedAt: null, reviewedBy: null, updatedAt: isoNow() });
      return clone(row);
    },
    async bulkMarkTransactionsReviewed({ householdId: targetHouseholdId, transactionIds, reviewedAt, reviewedBy }) {
      const now = isoNow();
      for (const transactionId of transactionIds) {
        const row = state.transactions.find((entry) => entry.householdId === targetHouseholdId && entry.id === transactionId);
        if (row) {
          Object.assign(row, { reviewedAt, reviewedBy: reviewedBy ?? null, updatedAt: now });
        }
      }
    },
    async deleteTransaction({ householdId: targetHouseholdId, transactionId }) {
      ...
      state.transactionSplits = state.transactionSplits.filter(
        (row) => !(row.householdId === targetHouseholdId && row.transactionId === transactionId),
      );
    },
    async insertTransactionSplit(payload) {
      const row = { id: uuid(), createdAt: isoNow(), updatedAt: isoNow(), ...payload };
      state.transactionSplits.push(row);
      return clone(row);
    },
    async listTransactionSplits({ householdId: targetHouseholdId, transactionId = null }) {
      return state.transactionSplits
        .filter((row) => row.householdId === targetHouseholdId)
        .filter((row) => (transactionId ? row.transactionId === transactionId : true))
        .map(clone);
    },
    async deleteTransactionSplits({ householdId: targetHouseholdId, transactionId }) {
      state.transactionSplits = state.transactionSplits.filter(
        (row) => !(row.householdId === targetHouseholdId && row.transactionId === transactionId),
      );
    },
    ...
    async insertDebtPaymentReconciliation({ householdId: targetHouseholdId, primaryPaymentId, duplicatePaymentId, status, matchType, confirmedBy = null }) {
      const now = isoNow();
      const row = {
        id: uuid(),
        workspaceId: targetHouseholdId,
      ...
      state.debtPaymentReconciliations.push(row);
      return clone(row);
    },
    async getDebtPaymentReconciliation({ householdId: targetHouseholdId, primaryPaymentId, duplicatePaymentId }) {
      return clone(
        state.debtPaymentReconciliations.find(
          (row) => row.householdId === targetHouseholdId
            && ((row.primaryPaymentId === primaryPaymentId && row.duplicatePaymentId === duplicatePaymentId)
              || (row.primaryPaymentId === duplicatePaymentId && row.duplicatePaymentId === primaryPaymentId)),
        ) ?? null,
      );
    },
    async listDebtPaymentReconciliations({ householdId: targetHouseholdId, debtId = null }) {
      const payments = state.debtPayments.filter((p) => p.householdId === targetHouseholdId);
      const paymentIds = new Set(payments.filter((p) => !debtId || p.debtId === debtId).map((p) => p.id));
      return state.debtPaymentReconciliations
        .filter((row) => row.householdId === targetHouseholdId
          && (paymentIds.has(row.primaryPaymentId) || paymentIds.has(row.duplicatePaymentId)))
        .map(clone);
    },
    async updateDebtPaymentReconciliation({ householdId: targetHouseholdId, reconciliationId, patch }) {
      const row = state.debtPaymentReconciliations.find(
        (entry) => entry.householdId === targetHouseholdId && entry.id === reconciliationId,
      );
      if (!row) return null;
      Object.assign(row, patch, { updatedAt: isoNow() });
      return clone(row);
    },
    async deleteDebtPaymentReconciliation({ householdId: targetHouseholdId, primaryPaymentId, duplicatePaymentId }) {
      const idx = state.debtPaymentReconciliations.findIndex(
        (row) => row.householdId === targetHouseholdId
          && ((row.primaryPaymentId === primaryPaymentId && row.duplicatePaymentId === duplicatePaymentId)
            || (row.primaryPaymentId === duplicatePaymentId && row.duplicatePaymentId === primaryPaymentId)),
      );
      if (idx === -1) return null;
      const [removed] = state.debtPaymentReconciliations.splice(idx, 1);
    ...
    async insertMonthClose(payload) {
      if (payload.status === 'CLOSED') {
        const householdKey = payload.householdId ?? payload.workspaceId;
        const duplicate = state.monthlyCloses.find(
          (r) =>
            (r.householdId === householdKey || r.workspaceId === householdKey) &&
            r.period === payload.period &&
            r.status === 'CLOSED',
        );
        if (duplicate) {
          const err = new Error('Month is already closed');
          err.status = 409;
          throw err;
        }
      }
      const row = { id: uuid(), createdAt: isoNow(), updatedAt: isoNow(), ...payload };
      state.monthlyCloses.push(row);
      return clone(row);
    },
    async getLatestMonthClose({ householdId: targetHouseholdId, period }) {
      const rows = state.monthlyCloses
        .filter((row) => (row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId) && row.period === period)
        .sort((a, b) => (b.version ?? 0) - (a.version ?? 0));
      return clone(rows[0] ?? null);
    },
    async listMonthCloses({ householdId: targetHouseholdId, period }) {
      return state.monthlyCloses
        .filter((row) => (row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId) && row.period === period)
        .sort((a, b) => (a.version ?? 0) - (b.version ?? 0))
        .map(clone);
    },
    async getMonthCloseById({ householdId: targetHouseholdId, closeId }) {
      return clone(state.monthlyCloses.find(
        (row) => (row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId) && row.id === closeId,
      ) ?? null);
    },
    async updateMonthClose({ householdId: targetHouseholdId, closeId, patch }) {
      const row = state.monthlyCloses.find(
        (entry) => (entry.householdId === targetHouseholdId || entry.workspaceId === targetHouseholdId) && entry.id === closeId,
      );
      if (!row) return null;
      Object.assign(row, patch, { updatedAt: isoNow() });
      return clone(row);
    ...
    async insertImportBatch(payload) {
      ...
      if (fileHash != null) {
        ...
        const conflict = state.importBatches.find(
          (b) => (b.workspaceId === wsId || b.householdId === wsId) && b.fileHash === fileHash,
        );
        if (conflict) {
          const err = new Error('duplicate key value violates unique constraint "idx_import_batches_workspace_file_hash"');
          err.code = '23505';
    ...
    async listImportBatches({ householdId: targetHouseholdId }) {
      return state.importBatches
        .filter((row) => row.householdId === targetHouseholdId)
        .sort((left, right) => compareValues(right.createdAt, left.createdAt) || compareValues(right.id, left.id))
        .map(clone);
    },
    async listImportedRowsForBatches({ householdId: targetHouseholdId, batchIds }) {
      const allowedBatchIds = new Set(batchIds);
      return state.importedRows
        .filter((row) => (row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId) && allowedBatchIds.has(row.batchId))
        .map(clone);
    },
    async listTransactionsByImportBatchIds({ householdId: targetHouseholdId, batchIds }) {
      const allowedBatchIds = new Set(batchIds);
      return state.transactions
        .filter((row) => row.householdId === targetHouseholdId && allowedBatchIds.has(row.importBatchId))
        .map(clone);
    },
    async listAccountReconciliationsForAccounts({ householdId: targetHouseholdId, accountIds }) {
      const allowedAccountIds = new Set(accountIds);
      return state.accountReconciliations
        .filter((row) =>
          (row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId)
          && allowedAccountIds.has(row.accountId))
        .map(clone);
    },
    ...
    async listUpcomingExpenses({ householdId: targetHouseholdId, status = null }) {
      return state.upcomingExpenses
        .filter((row) => row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId)
        .filter((row) => (status ? row.status === status : true))
        .sort((a, b) => (a.expectedDate < b.expectedDate ? -1 : a.expectedDate > b.expectedDate ? 1 : 0) || (a.id < b.id ? -1 : 1))
        .map(clone);
    },
    ...
    async getUpcomingExpenseById({ householdId: targetHouseholdId, expenseId }) {
      return clone(state.upcomingExpenses.find(
        (row) => (row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId) && row.id === expenseId,
      ) ?? null);
    },
    async updateUpcomingExpense({ householdId: targetHouseholdId, expenseId, patch }) {
      const row = state.upcomingExpenses.find(
        (entry) => (entry.householdId === targetHouseholdId || entry.workspaceId === targetHouseholdId) && entry.id === expenseId,
      );
      if (!row) return null;
      Object.assign(row, patch, { updatedAt: isoNow() });
      return clone(row);
    },
    async deleteUpcomingExpense({ householdId: targetHouseholdId, expenseId }) {
    ...
    async getAccountFreshnessContext({ householdId: targetHouseholdId }) {
      const accounts = state.financialAccounts.filter((a) => a.householdId === targetHouseholdId);
      const importBatches = accounts
        .map((a) => {
          const batch = state.importBatches
            .filter((b) => b.householdId === targetHouseholdId && b.accountId === a.id)
            .sort((x, y) => {
              const xDate = String(x.uploadedAt ?? x.createdAt ?? '');
              const yDate = String(y.uploadedAt ?? y.createdAt ?? '');
              return yDate.localeCompare(xDate);
            })[0];
          return batch ? { accountId: a.id, latestImportAt: batch.uploadedAt ?? batch.createdAt } : null;
        })
        .filter(Boolean);

      const acceptedReconciliations = state.accountReconciliations
        .filter((r) => r.householdId === targetHouseholdId && r.status === 'resolved')
        .map((r) => ({
          accountId: r.accountId,
          reconciledAt: r.resolvedAt,
        }));

      const unreviewedImportCount = state.importedRows
        .filter((row) => row.householdId === targetHouseholdId && row.status === 'pending_review')
        .length;
