...
const serverEnvSchema = z.object({
  ...
  // Must be http or https — javascript:, data:, ftp: and other schemes are rejected.
  RAF_APP_URL: z.string().url().refine(
    (val) => { try { const u = new URL(val); return u.protocol === 'https:' || u.protocol === 'http:'; } catch { return false; } },
    { message: 'RAF_APP_URL must be an http or https URL' },
  ).optional(),
});
