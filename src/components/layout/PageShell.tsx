...
export function PageShell({ eyebrow, title, description, actions, children }: PageShellProps) {
  return (
    <div>
      ...
      <header className="page-shell-header md:hidden mb-[16px]">
        <div className="min-w-0">
          {eyebrow ? <p className="text-[11px] font-[800] uppercase tracking-[0.14em] text-[var(--text-secondary)] mb-[6px]">{eyebrow}</p> : null}
          <h1 className="text-[25px] font-black leading-[1.1] tracking-[-0.045em] text-[var(--text-primary)] m-0">{title}</h1>
          {description ? <p className="mt-[7px] max-w-[670px] text-[11.5px] leading-[1.55] text-[var(--text-secondary)]">{description}</p> : null}
        </div>
